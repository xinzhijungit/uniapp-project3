...
app.post('/api/v1/search/entity', async (req, res) => {
  try {
    ...
    if (personInfo) {
      // 获取警情记录
      const caseRecords = await db.query(`
      ...
      // 获取户籍信息
      const householdInfo = await db.query('SELECT * FROM person_household WHERE id_card = ?', [personInfo.ZJHM])
      
      // 获取案底记录
      const personCriminalRecords = await db.query('SELECT * FROM criminal_record_db WHERE id_card = ?', [personInfo.ZJHM])
  ...
  try {
    ...
    if (entityId) {
      ...
      if (entityType === 'citizen') {
        const personResults = await db.query('SELECT BH, XM FROM FKD_BJR WHERE BH = ?', [entityId])
...
app.get('/api/v1/tags', async (req, res) => {
  try {
    ...
    const totalResult = await db.query('SELECT COUNT(*) as total FROM TAG_CONFIG_ontology')
    const total = totalResult[0].total
    
    const offset = (parseInt(page) - 1) * parseInt(size)
    const results = await db.query(`SELECT * FROM TAG_CONFIG_ontology ORDER BY CREATE_TIME DESC LIMIT ${parseInt(size)} OFFSET ${offset}`)
...
app.get('/api/v1/warning/events', async (req, res) => {
  try {
    ...
    const totalResult = await db.query('SELECT COUNT(*) as total FROM warning_event')
    const total = totalResult[0].total
    
    const offset = (parseInt(page) - 1) * parseInt(size)
    const results = await db.query(`SELECT * FROM warning_event ORDER BY TRIGGER_TIME DESC LIMIT ${parseInt(size)} OFFSET ${offset}`)
...
app.get('/api/v1/warning/indicators', async (req, res) => {
  try {
    ...
    const totalResult = await db.query('SELECT COUNT(*) as total FROM warning_indicator')
    const total = totalResult[0].total
    
    const offset = (parseInt(page) - 1) * parseInt(size)
    const results = await db.query(`SELECT * FROM warning_indicator ORDER BY create_time DESC LIMIT ${parseInt(size)} OFFSET ${offset}`)
...
app.get('/api/v1/warning/rules', async (req, res) => {
  try {
    ...
    const totalResult = await db.query('SELECT COUNT(*) as total FROM warning_rule')
    const total = totalResult[0].total
    
    const offset = (parseInt(page) - 1) * parseInt(size)
    const results = await db.query(`SELECT * FROM warning_rule ORDER BY create_time DESC LIMIT ${parseInt(size)} OFFSET ${offset}`)
...
app.get('/api/v1/qa/history', async (req, res) => {
  try {
    ...
    const offset = (parseInt(page) - 1) * parseInt(size)
    const results = await db.query(`SELECT * FROM qa_conversation ORDER BY create_time DESC LIMIT ${parseInt(size)} OFFSET ${offset}`)
